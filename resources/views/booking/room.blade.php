...
@push('styles')
    <link rel="stylesheet" href="{{ asset('css/booking_room.css') }}">
@endpush
...
@endif
    ...
    <form action="{{ route('razorpay.checkout') }}" method="POST" id="booking-form">
        ...
        <div class="alert alert-info" id="total-amount-info" style="display:none;">
            Total Amount: ₹<span id="total-amount"></span> ({{ $room->price_per_night }} per day)
        </div>

        <button class="btn btn-success mt-3" type="submit">Pay with Razorpay</button>
        <a href="{{ route('rooms.index') }}" class="btn btn-secondary mt-3">Cancel</a>
    </form>
</div>

<script>
    const pricePerDay = {{ $room->price_per_night }};
